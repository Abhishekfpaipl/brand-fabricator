    ...
    <div class="modal fade" id="ledgerChatModal" tabindex="-1" aria-labelledby="ledgerChatModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-fullscreen modal-dialog-scrollable">
            <div class="modal-content">
                <div v-if="ledger" class="d-flex flex-column justify-content-between h-100">
                    <div class="modal-header ps-0 py-2 shadow d-flex justify-content-between align-items-center">
                        <div class="d-flex justify-content-start align-items-center">
                            <router-link :to="'/running-orders/' + ledger.sid" class="btn">
                                <i class="bi bi-chevron-left"></i>
                            </router-link>
                            <span class="modal-title">{{ ledger.name }}</span>
                        </div>
                        <div class="d-flex aling-items-center">
                            <button type="button" class="btn" @click="toggleChatOpacity">
                                <i class="bi bi-eye"></i>
                            </button>
                            <button v-if="page <= pagination.lastPage" class="btn" @click="loadChats">
                                <i class="bi bi-arrow-clockwise pe-2"></i>
                            </button>
                        </div>
                    </div>
                    <div v-if="ledger.stock" class="modal-body p-0 bg-fullscreen reversed-list"
                        :style="{ 'background-image': 'url(' + ledger.stock.product.preview + ')' }"
                        style="max-height: calc(100vh - 110px); overflow-y: scroll;">
                        <ul class="list-group">
                            <li class="list-group-item bg-transparent border-0 py-1" v-for="(chat, msgIndex) in chats" :key="msgIndex">
...
export default {
    ...
    methods: {
        loadChats() {
            ...
            if (this.page > this.pagination.lastPage) return;
            ...
            navigator.mediaDevices.getUserMedia({ audio: true })
                .then((stream) => {
                    ...
                    this.mediaRecorder.onstop = async () => {
                        ...
                        formData.append('ledger_sid', this.ledger.sid);
